...
const handleVideoError = (error: unknown, next: (error: unknown) => void): void => {
...
videoRouter.post("/session", (_req, res, next) => {
  void createVideoSession().then((response) => res.json(response)).catch((error: unknown) => handleVideoError(error, next));
});
